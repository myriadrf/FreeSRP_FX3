...
// Returns number of valid bytes written to send_buf
int process_command(struct ad9361_rf_phy *phy, uint8_t *recv_buf, uint8_t *send_buf)
{
    if(phy == NULL)
    {
        for(int i = 0; i < 11; i++)
	...
	if(cmd_id >= cmd_no)
	{
        ...
        send_buf[10] = AD9364_CMD_ERR_INVALID_COMMAND;
        return 11;
	}
	...
	if(param_no > 1)
	{
        ...
        send_buf[10] = AD9364_CMD_ERR_INVALID_PARAMETERS;
        return 11;
	}
	...
	send_buf[0] = (uint8_t) cmd_id;
	send_buf[1] = (uint8_t) ((error == CMD_OK) ? 1 : 0);
	send_buf[10] = (uint8_t) error;
...
// Takes pointer to recv_buf, which must be a 10 byte long array containing a command.
// Populates resp_buf, which should be 10 bytes as well.
int ad9364_cmd(uint8_t *recv_buf, uint8_t *resp_buf)
{
	return process_command(ad9361_phy, recv_buf, resp_buf);
}